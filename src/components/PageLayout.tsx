      ...
      {isMobile ? (
        <>
          ...
          >
            {({ close }) => (
              <SideMenu
                className="flex-container h-screen"
      ...
      <FadeIn>
        {isLowRpcPerformance && (
          <div className="bg-[#dacc363f] text-center text-[#D8CB39] text-sm mobile:text-xs px-4 py-1">
            The Solana network is experiencing congestion or reduced
            performance. Transactions may fail to send or confirm.
          </div>
        )}
      </FadeIn>
    </div>
        ...
        {versionRefreshData === "too-old" && (
          <Row className="w-[min(400px,100%)]  m-auto justify-center items-center py-4 px-6 mobile:py-3 mb-8 mobile:mb-2 rounded-xl ring-1.5 ring-inset ring-[#D8CB39] bg-[#1B1659]">
            <div className="text-[#C4D6FF] text-sm font-medium">
              ...
              <span
                className="clickable text-[#D8CB39] font-bold"
...
function MigrateBubble() {
  const noNeedAtaTokenMints = useAsyncMemo(
    async () => (await jFetch("/no-ata-check-token-mints.json")) as string[]
  );
  const rawTokenAccounts = useWallet((s) => s.allTokenAccounts);
  const connected = useWallet((s) => s.connected);

  const needMigrate = useMemo(() => {
    const isInWhiteList = (mint: string) =>
      (noNeedAtaTokenMints ?? []).includes(String(mint));
    ...
    if (needMigrate) {
      useNotification.getState().logWarning(
        ...
        <div>
          ...
          <Link href="https://v1.raydium.io/migrate/">migrate</Link> to new ATA
          account
        </div>
      );
    }
  ...
  return (
    <Grid className="pc:grid-cols-[auto,1fr,auto] mobile:grid-cols-[auto,1fr] gap-3 justify-between items-center py-6 px-8 mb-10 rounded-xl ring-1.5 ring-inset ring-[#DA2EEF] bg-[#1B1659]">
      <Icon
        className="text-[#DA2EEF] self-start mobile:row-span-2"
      ...
      <Col className="gap-2">
        ...
        <div className="text-[#C4D6FF] text-sm font-medium">
          Your wallet have old normal tokenAccount. Please migrate to ATA
          tokenAccount for safer transaction.
        </div>
      </Col>
      <Row className="gap-8">
        <Button
        ...
        <Button
          ...
          onClick={() =>
            linkTo(
              "https://raydium.gitbook.io/raydium/updates/associated-token-account-migration"
            )
          }
        >
          Learn More
...
function Navbar({
  ...
  const mobileNavContent = (
    <Grid className="grid-cols-3 items-center">
      <div
        className="frosted-glass-teal rounded-lg p-2 clickable justify-self-start"
        onClick={onOpenMenu}
      >
        <Icon
          className="w-4 h-4"
          iconClassName="w-4 h-4"
          iconSrc="/icons/msic-menu.svg"
        />
      </div>
      ...
      {barTitle ? (
        <div
          onClick={onOpenMenu}
          className="text-lg font-semibold place-self-center text-white -mb-1"
        >
          {barTitle}
        </div>
      ) : (
  ...
  return (
    <nav
      className={twMerge(
        "select-none text-white px-12 py-4 mobile:px-5 mobile:py-3 transition-all",
        className
      )}
      style={style}
    >
    ...
    setCssVarible(
      ...
      sideMenuRef.current
        ? Math.min(
            ...
            sideMenuRef.current.clientHeight
          )
        : 0
    );
  }, [sideMenuRef]);
      ...
      <Col
        ...
        className={
          isMobile
            ...
            : twMerge(
                `w-full overflow-y-auto mobile:w-48 mobile:rounded-tr-2xl mobile:rounded-br-2xl`,
                className
              )
        }
        style={{
          background: isMobile
            ...
            : undefined,
          boxShadow: isMobile
            ? "8px 0px 48px rgba(171, 196, 255, 0.12)"
            : undefined,
        }}
      >
        {isMobile && (
        ...
        {isMobile ? (
          ...
          <Row className="justify-between flex-1 overflow-hidden">
            <div>
              <hr className="mx-8 border-[rgba(57,208,216,0.16)] mb-3" />
            ...
            <div className="flex-grow overflow-y-auto p-4 m-2 grid grid-cols-4 gap-2">
              {/* <LinkItem icon="/icons/entry-icon-trade.svg" href="https://dex.raydium.io/">
              Trading
            </LinkItem> */}
              <LinkItem
            ...
            <div className="text-sm m-2 opacity-20 hover:opacity-100 transition font-medium text-[#abc4ff] whitespace-nowrap flex flex-row justify-end items-end">
              <div className="text-center">Copyright © 2022</div>
            </div>
          </Row>
        )}
...
function OptionItem({
  ...
  return (
    ...
    >
      <Row
        className="items-center w-full mobile:justify-center"
        onClick={onClick}
      >
        <Icon
...
function SettingPopover() {
  ...
  const slippageToleranceState = useAppSettings(
    (s) => s.slippageToleranceState
  );
  return (
    <div className="py-5 px-6">
      <Row className="items-center mb-3 mobile:mb-6 gap-2">
        <div className="text-[rgba(171,196,255,0.5)] text-xs mobile:text-sm">
          SLIPPAGE TOLERANCE
        </div>
        <Tooltip placement="bottom-right">
          <Icon
            ...
            className="cursor-help text-[rgba(171,196,255,0.5)]"
          />
          <Tooltip.Panel>
      ...
      <Row className="gap-3 justify-between">
        ...
        >
          <Row>
            <Input
              className="w-[32px]"
              value={toString(mul(slippageTolerance, 100), {
  ...
  return (
    <PageLayoutPopoverDrawer
      renderPopoverContent={({ close }) => (
        <RpcConnectionPanelPopover close={close} />
      )}
    >
      <RpcConnectionFace />
    </PageLayoutPopoverDrawer>
...
function RpcConnectionFace() {
  ...
  const loadingCustomizedEndPoint = useConnection(
    (s) => s.loadingCustomizedEndPoint
  );
      ...
      <Row className="items-center w-full mobile:justify-center">
        ...
        >
          {currentEndPoint
            ? isLoading
              ? `RPC (${
                  (loadingCustomizedEndPoint?.name ??
                    extractConnectionName(
                      loadingCustomizedEndPoint?.url ?? ""
                    )) ||
                  ""
                })`
              : `RPC (${
                  (currentEndPoint?.name ??
                    extractConnectionName(currentEndPoint.url)) ||
                  ""
                })`
            : "--"}
        </span>
...
function RpcConnectionPanelPopover({
  close: closePanel,
}: {
  close: () => void;
}) {
          ...
          return (
            ...
            >
              <Row className="items-center w-full">
                ...
                >
                  {endPoint.name ?? "--"}
                  {endPoint.url === autoChoosedEndPoint?.url && (
                    <Badge className="self-center ml-2">recommended</Badge>
                  )}
